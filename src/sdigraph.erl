...
%% %CopyrightEnd%
%%
%% Modified by Gary Hai <gary@XL59.com> for public access and fast insertion.
...
%%  acyclic | cyclic
%%
%%  default is [cyclic,public]
-type d_protection() :: 'private' | 'protected' | 'public'.
...
new(Type) ->
    case check_type(Type, public, []) of
	{public, Ts} ->
            Opts = [public, {write_concurrency,true}, {read_concurrency,true}],
	    V = new_shared_tab(vertices, [set | Opts]),
	    E = new_shared_tab(edges, [set | Opts]),
%	    N = new_shared_tab(neighbours, [bag | Opts]);
	    N = new_shared_tab(neighbours, [duplicate_bag | Opts]);
	{Access, Ts} ->
	    V = ets:new(vertices, [set, Access]),
	    E = ets:new(edges, [set, Access]),
%	    N = ets:new(neighbours, [bag, Access])
 	    N = ets:new(neighbours, [duplicate_bag, Access])
    end,
    ets:insert(N, [{'$vid', 0}, {'$eid', 0}]),
    set_type(Ts, #digraph{vtab=V, etab=E, ntab=N}).

%%
%% global keeper for public access ets
%%
-define(GLOBAL_ETS_KEEPER, ouriah669f9632).
new_shared_tab(Name, Options) ->
    true = proplists:get_value(public, Options),
    case whereis(?GLOBAL_ETS_KEEPER) of
        undefined ->
            Keeper = spawn(fun keeper_proc/0),
            true = register(?GLOBAL_ETS_KEEPER, Keeper);
        Keeper ->
            Keeper
    end,
    Keeper ! {self(), '$_ets_new', Name, Options},
    receive
        {'$_ets_ok', Tab} ->
            Tab
    after
...
keeper_proc() ->
    receive
        {From, '$_ets_new', Name, Options} ->
            try
                Tab = ets:new(Name, Options),
                erlang:send(From, {'$_ets_ok', Tab})
            catch
                _:_ ->
                    keep_running
            end;
...
add_edge(G, E, V1, V2, D) ->
    case edge(G, E) of
        ...
        {E, V1, V2, _} ->
            ets:insert(G#digraph.etab, {E, V1, V2, D}),
            E;
        _BadEdge ->
