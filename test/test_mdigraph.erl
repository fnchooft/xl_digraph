     ...
     fun(G) ->
	     {inorder,
	      [
	       ...
	       ?_assertMatch({error, {bad_vertex, "not_exist"}}, mdigraph:add_edge(G, "foo", "not_exist"))
	      ]
	     }
     end}.
...
share_db_test_() ->
    {setup, fun() -> mnesia:start(), G = mdigraph:new(), G end, fun(_G) -> mnesia:stop() end,  
     fun(G) ->
             Tbls = mdigraph:info(G, tables),
             G1 = mdigraph:new(Tbls),
	     {inorder,
	      [
	       ?_assertMatch("foo",          mdigraph:add_vertex(G, "foo")),
	       ?_assertMatch("bar",          mdigraph:add_vertex(G1, "bar")),
	       ?_assertMatch("next",         mdigraph:add_vertex(G, "next")),
	       ?_assertEqual(3,              mdigraph:no_vertices(G)),
	       ?_assertEqual(3,              mdigraph:no_vertices(G1)),
	       ?_assertMatch(['$e'|0],         mdigraph:add_edge(G1, "foo", "bar")),
	       ?_assertMatch(['$e'|1],         mdigraph:add_edge(G, "foo", "next")),
	       ?_assertEqual(2,              mdigraph:no_edges(G)),
	       ?_assertEqual(2,              mdigraph:no_edges(G1)),
	       ?_assertMatch({error, {bad_vertex, "not_exist"}}, mdigraph:add_edge(G, "foo", "not_exist"))
	      ]
	     }
     end}.
...
persistence_test() ->
    ...
    G = mdigraph:new(Opts),
    ?assertMatch("foo",          mdigraph:add_vertex(G, "foo")),
    ?assertMatch("bar",          mdigraph:add_vertex(G, "bar")),
    ?assertMatch("next",         mdigraph:add_vertex(G, "next")),
    ?assertMatch(['$e'|0],         mdigraph:add_edge(G, "foo", "bar")),
    ?assertMatch(['$e'|1],         mdigraph:add_edge(G, "foo", "next")),
    ?assertMatch({error, {bad_vertex, "not_exist"}}, mdigraph:add_edge(G, "foo", "not_exist")),
    %% stop mnesia
    ?assertMatch(stopped, mnesia:stop()),
    G1 = mdigraph:new(Opts),
    ?assertMatch("reload",         mdigraph:add_vertex(G1, "reload")),
    ?assertEqual(4,              mdigraph:no_vertices(G1)),
    ?assertMatch(['$e'|_],         mdigraph:add_edge(G1, "foo", "reload")),
    ?assertEqual(3,              mdigraph:no_edges(G1)),
    mnesia:stop(), 
    mnesia:delete_schema([node()]).
