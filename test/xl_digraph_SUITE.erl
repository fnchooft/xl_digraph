...
init_per_testcase(_TestCase, Config) ->
    ...
    %% init digraph and xl_digraph with same data
    %% add vertices
    [xl_digraph:add_vertex(MG, V) || V <- Vertices],
    [digraph:add_vertex(DG, V) || V <- Vertices],
    %% add edges
    [xl_digraph:add_edge(MG, V1, V2) || {V1, V2} <- Edges],
...
in_out_edges(Config) ->
    ...
    Vertices = ?config(vertices, Config),
    %% in degree
    MG_In_E = [ {V, xl_digraph:in_edges(MG, V)} || V <- Vertices ],
    DG_In_E = [ {V, digraph:in_edges(DG, V)} || V <- Vertices ],
    ct:log("-> In_edges, ~p, ~p ", [MG_In_E, DG_In_E]),
    MG_In_E = DG_In_E,
    %% out degree
    MG_Out_E = [ {V, xl_digraph:out_edges(MG, V)} || V <- Vertices ],
...
del_vertex(Config) ->
    ...
    %% delete two
    Exp_v2 = Exp_V - 2,
    true = xl_digraph:del_vertices(MG, ["E", "D"]),
    true = digraph:del_vertices(DG,  ["E", "D"]),
    Exp_V2 = xl_digraph:no_vertices(MG),
...
del_edge(Config) ->
    ...
    DG_E = lists:sort(digraph:edges(DG)),
    %% delete one edge
    true = xl_digraph:del_edge(MG, hd(MG_E)),
    true = digraph:del_edge(DG, hd(DG_E)),
    MG_E_2 = lists:sort(xl_digraph:edges(MG)),
    DG_E_2 = lists:sort(digraph:edges(DG)),
    ct:log("-> edges, ~p, ~p ", [MG_E_2, DG_E_2]),
    MG_E_2 = DG_E_2,
    %% delete in the middle
    true = xl_digraph:del_edge(MG, lists:nth(3, MG_E)),
    true = digraph:del_edge(DG, lists:nth(3, DG_E)),
    MG_E_3 = lists:sort(xl_digraph:edges(MG)),
    DG_E_3 = lists:sort(digraph:edges(DG)),
    ct:log("-> edges, ~p, ~p ", [MG_E_3, DG_E_3]),
    MG_E_3 = DG_E_3,
    %% check number of edges
    MG_N = xl_digraph:no_edges(MG),
...
path(Config) ->
    ...
    DG = ?config(dg, Config),
    %% source
    MG_Path = xl_digraph:get_path(MG, "A", "E"),
    DG_Path = digraph:get_path(DG, "A", "E"),
    ct:log("-> path, ~p, ~p ", [MG_Path, DG_Path]),
    MG_Path = DG_Path,

    MG_S_Path = xl_digraph:get_short_path(MG, "A", "E"),
...
del_path(Config) ->
    ...
    DG = ?config(dg, Config),
    %% source
    true = xl_digraph:del_path(MG, "A", "B"),
    true = digraph:del_path(DG, "A", "B"),
    %% check is alternative path exist 
    MG_P = xl_digraph:get_path(MG, "A", "D"),
    ...
    true = xl_digraph:del_path(MG, "D", "E"),
    ...
    MG_Path = xl_digraph:get_path(MG, "A", "F"),
